...
console.log("Javascript is working!") // test
...
const percentOf = function (number1, number2) {
  const total = `${(number1 / number2) * 100}%`;
...
const areaOfCircle = function (radius) {
  const total = (Math.PI * radius * radius).toFixed(2);
  console.log(`The area for a circle with radius ${ radius } is ${ total }.`)
  return total;
};
...
const calculator = function(number) {
  ...
  const percent = percentOf(squared);
}
